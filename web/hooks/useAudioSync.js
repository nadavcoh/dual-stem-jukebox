 ...
 * directly from the audio engine's own clock for one deck
 * (engineRef.current.getPlayheadSeconds(slot)).
 *
 ...
 *
 * Call this once per deck (it's per-slot now, since A and B each have their
 * own independent playhead):
 *
 *   const playheadRefA = useAudioSync(engineRef, "a", totalDurationA);
 *   const playheadRefB = useAudioSync(engineRef, "b", totalDurationB);
 *   <div ref={playheadRefA}>
 *     <div style={{ left: "calc(var(--playhead-pct, 0) * 100%)" }} />
 *   </div>
 */
export function useAudioSync(engineRef, slot, totalDurationSeconds) {
  ...
  useEffect(() => {
    function tick() {
      ...
      if (engine && el) {
        const seconds = engine.getPlayheadSeconds(slot);
  ...
  }, [engineRef, slot, totalDurationSeconds]);
